...
export class GameView extends View {
	...
	this.#ball = new Ball(
	    ...
	    -2, 2, 20);
	// ball_dx: -1.5, ball_dy: 1.5 for easy mode
	// ball_dx: -3, ball_dy: 3 for normal mode
	...
	// speed: 75 for easy mode
	// speed: 50 for normal mode
	...
	if (this.#AIPlayer == true) {
	    this.#aiUpdateTime = Date.now() - this.#updateMs;
	    if (this.#AIMode == "normal") {
		this.#aiSocket = new WebSocket(
		    "ws://" + window.location.host + "/ws/pong/ai_player/"
		);
		let that = this;
		this.#aiSocket.onmessage = function(e) {
		    const data = JSON.parse(e.data);
		    that.#rightPaddle.dy = data.action * that.#rightPaddle.speed;
		};
	    }
	}
	...
	if (this.#ball.dx < 0) {
	    const idealY = this.context.canvas.height / 2;
	    if (paddleCenterY < idealY)
	    ...
	    let ballDy = this.#ball.dy;
	    for (;ballX < this.#rightPaddle.x;) {
		ballX += ballDx;
		ballY += ballDy;
		if (ballY <= 0 || ballY + this.#ball.sideLength >= this.context.canvas.height)
		    ballDy = -ballDy;
	    }
    ...
    #sendStatesToAIServer() {
	let reward = this.#rightPaddleHitNum >= 1 ? 1 : 0;
	reward -= this.#scores.leftScore.value > this.#scores.leftScore.preValue ? 1 : 0;
	this.#aiSocket.send(JSON.stringify({
	    "ball_x": this.#ball.x,
	    "ball_y": this.#ball.y,
	    "right_paddle_y": this.#rightPaddle.y,
	    "ball_dx": this.#ball.dx,
	    "ball_dy": this.#ball.dy,
	    "reward": reward,
	    "done": this.#scores.rightScore.value > this.#scores.rightScore.preValue ||
		this.#scores.leftScore.value > this.#scores.leftScore.preValue
	}));
	this.#ball.preX = this.#ball.x;
	this.#ball.preY = this.#ball.y;
	this.#leftPaddle.preY = this.#leftPaddle.y;
	this.#rightPaddle.preY = this.#rightPaddle.y;
	this.#scores.leftScore.preValue = this.#scores.leftScore.value;
	this.#scores.rightScore.preValue = this.#scores.rightScore.value;
	this.#rightPaddleHitNum = 0;
    }
  ...
  update() {
      ...
      if (this.#AIPlayer == true) {
	  if (Date.now() - this.#aiUpdateTime >= this.#updateMs) {
	      // pesudo ai opponent that follow subject requirements
	      if (this.#AIMode == "easy")
		  this.#AIPredictMove();
	      // true ai opponent, but does not follow subject requirements
	      else if (this.#AIMode == "normal")
		  this.#sendStatesToAIServer();
